...
public class FixtureGenerator <T extends Object>{
    public List<List<Fixture<T>>> getFixtures(List<T> teams, boolean includeReverseFixtures) {
        ...
        List<List<Fixture<T>>> rounds = new LinkedList<>();

        for (int round = 0; round < totalRounds; round++) {
            List<Fixture<T>> fixtures = new LinkedList<>();
        ...
        List<List<Fixture<T>>> interleaved = new LinkedList<>();
        ...
        if(includeReverseFixtures){
            List<List<Fixture<T>>> reverseFixtures = new LinkedList<>();
            for(List<Fixture<T>> round: rounds){
                List<Fixture<T>> reverseRound = new LinkedList<>();
